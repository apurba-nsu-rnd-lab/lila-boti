...
with open("./bnhtrdtrain/init3/inv_grapheme_dict_synth.pickle", 'wb') as handle:
...
# exmodel = model_vgg.get_crnn(len(inv_grapheme_dict)+1,qatconfig=None)
# exmodel = exmodel.to(device)
student_model = student_model.to(device)
#torch.save(student_model.state_dict(), './bwtrain/init2/init.pth')
#from 7
#exmodel.load_state_dict(torch.load('./bwtrain/init2/init.pth', map_location=device))
...
train_loader = DataLoader(train_dataset, batch_size=train_batch_s, shuffle=False, num_workers=8)
...
optimizer = optim.Adam(filter(lambda p: p.requires_grad, student_model.parameters()), 0.001, weight_decay=1e-05)
# torch.save({
            
#             'model_state_dict': student_model.state_dict(),
#             'optimizer_state_dict': optimizer.state_dict(),
            
#             }, './bnhtrdtrain/init3/init.pth')
checkpoint = torch.load('./bnhtrdtrain/init3/init.pth')
student_model.load_state_dict(checkpoint['model_state_dict'])
optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

def train(epochs=30, lr = 0.001):
